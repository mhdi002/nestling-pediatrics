...
(function () {
  ...
  function t(key) {
    ...
    return pack[key] != null ? pack[key] : ((window.NESTLING_I18N && window.NESTLING_I18N.en[key]) || key);
  }
  ...
  function renderChildChip() {
    ...
    if (state.activeChild && state.activeChild.name) {
      ...
      chip.title = `Active: ${state.activeChild.name}`;
    } else {
      chip.hidden = true;
      chip.textContent = "";
    }
    ...
    try {
      ...
      const chartsHtml = overlays.length
        ? `<div class="dossier-charts">${overlays
            .map(
              (o) =>
                `<a href="${escapeHtml(o.url)}" target="_blank" rel="noopener"><img class="overlay-img" src="${escapeHtml(
                  o.url
                )}" alt="chart" /></a>`
            )
            .join("")}</div>`
  ...
  async function api(path, options = {}) {
    ...
    try {
      ...
      const e = new Error("Can't reach the server. Is Nestling running?");
      e.cause = err;
      throw e;
    }
    let data = null;
  ...
  async function checkHealth() {
    ...
    try {
      ...
      el.textContent = "Offline — UI works; connect API when ready.";
  ...
  async function loadChildren() {
    ...
    try {
      ...
      if (!state.children.length) {
        list.innerHTML = `<p class="muted">No children yet — add one below.</p>`;
        return;
      }
    ...
    list.innerHTML = sessions
      .map((s) => {
        ...
        return `<button type="button" class="history-item" data-sid="${escapeHtml(
          s.session_id
        )}"><strong>${escapeHtml(title || "Chat")}</strong><span class="muted">${escapeHtml(
          meta
        )}</span></button>`;
      })
      .join("");
  ...
  function appendToolResult(payload) {
    ...
    list.forEach((tr) => {
      ...
      block.innerHTML = `<img class="overlay-img" src="${escapeHtml(img)}" alt="Growth chart overlay" />`;
      thread.appendChild(block);
    });
    const topImg = overlaySrc(payload);
    if (topImg && !thread.querySelector(`img.overlay-img[src="${CSS.escape(topImg)}"]`)) {
      const imgEl = document.createElement("img");
      imgEl.className = "overlay-img";
      imgEl.src = topImg;
      imgEl.alt = "Growth chart overlay";
      thread.appendChild(imgEl);
    }
    thread.scrollTop = thread.scrollHeight;
  }
  ...
  function wireChat() {
    ...
    if (attachBtn && fileInput && !attachBtn.dataset.wired) {
      ...
      fileInput.addEventListener("change", async () => {
        ...
        const caption =
          input.value.trim() ||
          "Please look at this photo of my child's skin. What should I watch for?";
        const btn = $("#chat-send");
        appendBubble("user", `📷 ${caption}`);
        input.value = "";
        input.style.height = "auto";
        setLoading(btn, true);
        const scene = appendBubble("tool-scene", t("usingTools"));
        try {
          await ensureChatSession();
          const fd = new FormData();
          fd.append("image", file);
          fd.append("message", caption);
          fd.append("session_id", state.chatSessionId || "");
          fd.append("ui_lang", state.lang);
          const childId =
            (state.activeChild && (state.activeChild.child_id || state.activeChild.id)) || "";
          if (childId) fd.append("child_id", childId);
          const res = await fetch("/api/chat/vision", { method: "POST", body: fd });
          const data = await res.json().catch(() => ({}));
          if (!res.ok) {
            const detail = (data.detail && (data.detail.detail || data.detail.error)) || res.statusText;
            throw new Error(detail || "Vision chat failed");
          }
          scene.remove();
          if (data.session_id) {
            state.chatSessionId = data.session_id;
          ...
          await streamAssistantBubble(data.reply || "Done.");
          toast(t("photoSent"), "ok");
        } catch (err) {
          scene.remove();
          appendBubble("system", err.message || String(err));
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const reply =
          data.reply || data.message || data.response || data.answer || "Done.";
        await streamAssistantBubble(reply);
        if (data.tool_results || overlaySrc(data)) {
        ...
        if (String(err.message || "").includes("session_not_found") || err.status === 404) {
          ...
          try {
            ...
            if (data.tool_results || overlaySrc(data)) appendToolResult(data);
            return;
          } catch (err2) {
    ...
    $("#growth-form").addEventListener("submit", async (e) => {
      ...
      try {
        ...
        out.innerHTML = `
          ...
          <div class="stat-row">
            <div class="stat"><span class="val">${escapeHtml(centile)}</span><span class="lbl">Centile</span></div>
            <div class="stat"><span class="val">${escapeHtml(z)}</span><span class="lbl">Z-score</span></div>
            <div class="stat"><span class="val">${escapeHtml(String(data.value ?? body.value))}</span><span class="lbl">${escapeHtml(data.units?.[data.measure] || data.measure || "Value")}</span></div>
          </div>
          ${data.summary ? `<p>${escapeHtml(data.summary)}</p>` : ""}
          ${img ? `<img class="overlay-img" src="${escapeHtml(img)}" alt="Growth chart with overlay" />` : ""}
        `;
        out.hidden = false;
  ...
  function showScreeningPicker() {
    ...
    $("#screening-title").textContent = "Screening";
    $("#screening-sub").textContent = "Pick a questionnaire for your child’s age.";
    $("#screening-back").href = "#/";
    $("#screening-back").textContent = "← Home";
  }
      ...
      (Array.isArray(domains) ? domains : []).forEach((dom) => {
        ...
        (dom.questions || []).forEach((q) => {
          items.push({
            ...
            options: [
              { value: "yes", label: "Yes", cls: "yes" },
              { value: "sometimes", label: "Sometimes", cls: "sometimes" },
              { value: "not_yet", label: "Not yet", cls: "not_yet" },
            ],
          });
        });
      });
  ...
  async function startAsq(age) {
    try {
      toast(`Loading ASQ ${age}m…`);
      const data = await api(`/asq/${age}/questions`);
      const items = normalizeQuestions(data, "asq");
      if (!items.length) throw new Error("No questions returned for this age.");
      state.quiz = {
        kind: "asq",
        age,
        items,
      ...
      $("#screening-title").textContent = `ASQ · ${age} months`;
      $("#screening-sub").textContent = "Answer based on what you’ve seen your child do.";
      beginQuiz();
    } catch (err) {
      toast(err.message, "error");
    }
  ...
  async function startMchat() {
    try {
      toast("Loading M-CHAT-R…");
      const data = await api("/mchat/questions");
      const items = normalizeQuestions(data, "mchat");
      if (!items.length) throw new Error("No M-CHAT questions returned.");
      state.quiz = {
        kind: "mchat",
        items,
        index: 0,
      ...
      $("#screening-title").textContent = "M-CHAT-R";
      $("#screening-sub").textContent = "Think about what your child usually does.";
      beginQuiz();
    } catch (err) {
      toast(err.message, "error");
    }
  ...
  function beginQuiz() {
    ...
    back.href = "#/screening";
    back.textContent = "← Questionnaires";
    renderQuizStep();
  }
  ...
  function renderQuizStep() {
    ...
    $("#quiz-meta").textContent = `Question ${q.index + 1} of ${total}`;
    ...
    $("#quiz-next").textContent = q.index === q.items.length - 1 ? "See results" : "Next";
  }
  ...
  function renderAsqReport(data) {
    ...
    report.innerHTML = `
      <h3>ASQ report</h3>
      <span class="badge ${badge}">${needs ? "Referral suggested" : "No domain below cutoff"}</span>
      <p>${escapeHtml(summary)}</p>
      <ul class="domain-list">${rows}</ul>
      <button type="button" class="btn btn-secondary btn-block" id="screening-again" style="margin-top:1rem">Another questionnaire</button>
    `;
    report.hidden = false;
  ...
  function renderMchatReport(data) {
    ...
    report.innerHTML = `
      <h3>M-CHAT-R report</h3>
      <span class="badge ${badge}">${escapeHtml(risk || "—")} risk</span>
      <div class="stat-row">
        <div class="stat"><span class="val">${escapeHtml(String(result.total_failed ?? data.total_failed ?? "—"))}</span><span class="lbl">Failed items</span></div>
      </div>
      <p>${escapeHtml(summary)}</p>
